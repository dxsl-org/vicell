    ...
    steps:
      ...
      - name: Update full CHANGELOG.md
        run: |
          ...
          git config user.name "DXSL"
          git config user.email "ci@Cellos"
      ...
      - name: Create GitHub Release
        ...
        with:
          ...
          files: target/riscv64gc-unknown-none-elf/release/Cellos-kernel
"""

cliff_toml = """[changelog]
header = "# Changelog\\n\\nAll notable changes to Cellos.\\n"
body = \"\"\"
...
contributing_md = """# Contributing to Cellos

Welcome! Cellos is a `no_std` Rust OS with a Cellular Single Address Space architecture.
...
- Lua 5.4 C binding with Cellos glue layer
- STRIDE security model documentation + cargo-geiger CI gate
"""

llms_txt = """# Cellos

> Cellos is a Rust no_std OS using Cellular Single Address Space + Language-Based Isolation.

## Core

- [Architecture Overview](docs/ARCHITECTURE.md): System design and Cell lifecycle
- [Coding Guide](docs/CODING_GUIDE.md): How to write Cellos code
...
python3 -c "
import os, re
lines = ['# Cellos', '', '> Cellos is a Rust no_std OS using Cellular SAS + LBI.', '', '## Docs', '']
