...
/// Blit ASCII `text` into `pixels` at screen position `(x, y)`.
///
/// - `pixels`: BGRA8888 pixel buffer (e.g. from `ViSurface::pixels_mut()`).
/// - `stride`: bytes per row = `surface.stride()`.
/// - `x`, `y`: top-left corner in surface coordinates.
/// - `color`: packed `0xBBGGRRAA` (BGRA order to match the buffer).
///
