...
- CORRECTION: research's test used `BOOT_TIMEOUT` for `wait_for("ViCell >", BOOT_TIMEOUT)` then small
...
**Test flow:**
```
boot → wait_for("ViCell >", 40)
     → send_line("echo PHASE_C_WRITE > /tmp/test.txt")
     → wait_for("ViCell >", CMD_TIMEOUT)        (prompt returns = command done)
...
| Prompt string differs ("ViCell >" vs "ViCell > ") | Low | Low | Reuse exact literal from boots_to_shell_prompt (boot.rs:64). |
