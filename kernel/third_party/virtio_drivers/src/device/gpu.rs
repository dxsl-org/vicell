...
impl<H: Hal, T: Transport> VirtIOGpu<H, T> {
    ...
    /// Flush a sub-rectangle of the framebuffer to screen.
    ///
    /// `offset` is the byte offset of the top-left corner of the rectangle in
    /// the guest backing store: `(y * full_width + x) * 4`.
    ///
    /// This is a Cellos-local extension to the upstream virtio-drivers crate
    /// (vendored at kernel/third_party/virtio_drivers). Upstream version: 0.7.5.
    pub fn flush_rect(&mut self, x: u32, y: u32, w: u32, h: u32, offset: u64) -> Result {
    ...
    /// Read the device's pending events bitmask from config space.
    ///
    /// Bit 0 (`EVENT_DISPLAY`) is set when display configuration has changed.
    /// Cellos-local extension — vendored at kernel/third_party/virtio_drivers.
    pub fn read_events(&self) -> u32 {
        // SAFETY: config_space returns a pointer to MMIO config space validated
        // by the transport during device initialization.
        let config = self.transport.config_space::<Config>().unwrap();
        unsafe { volread!(config, events_read) }
    }

    /// Acknowledge and clear pending events in the device config space.
    ///
    /// Write `bits` to `events_clear` to clear matching bits in `events_read`.
    /// Cellos-local extension — vendored at kernel/third_party/virtio_drivers.
    pub fn clear_events(&mut self, bits: u32) {
        use crate::volatile::volwrite;
        // SAFETY: config_space_mut returns a writable pointer to MMIO config space.
        let config = self.transport.config_space::<Config>().unwrap();
        unsafe { volwrite!(config, events_clear, bits) }
    }
