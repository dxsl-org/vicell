...
/// Sequential spawn throughput.  PASS iff ≥ 20 tasks/sec.
///
/// Tests that 2-hart scheduling keeps spawn+compute+exit overhead bounded.
fn measure_spawn_rate() -> Option<bool> {
    const N: u64 = 8;
    const TARGET: u64 = 20; // 50 ms per full lifecycle on QEMU TCG
