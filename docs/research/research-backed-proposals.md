# ViCell — Research-Backed Feature Proposals
...
| # | Vấn đề đã xác nhận | Nguồn | Liên quan ViCell |
|---|---|---|---|
...
**Giá trị thực tế**: ViCell có thể claim là RTOS đầu tiên có DMA safety primitive không cần IOMMU/SMMU phần cứng — chạy được trên Cortex-M, RISC-V MCU không có SMMU. Cụ thể hơn và thực tế hơn bất kỳ thứ gì Zephyr có.
...
### ViCell có thể làm tốt hơn FlexOS
ViCell đã có Cell manifest + load-time linking. Đề xuất thêm `isolation_level` vào manifest:
...
Điều FlexOS không làm được: **thay đổi isolation level của một Cell tại runtime** mà không restart toàn hệ thống. ViCell có thể: hot-swap Cell với manifest mới có `isolation_level` khác.
...
Unishyper dùng Intel MPK (Memory Protection Keys) trên x86_64 để bổ sung cho Rust. Vấn đề: **MPK không có trên ARM/RISC-V** — đây là những target chính của ViCell G1.
...
### ViCell đề xuất: Software Zone
Vì không có MPK trên ARM/RISC-V, ViCell dùng phương pháp software:
...
**Giá trị**: Không cần CHERI hardware, không cần MPK, chạy được trên mọi ARM64/RV64 ViCell target. Defense-in-depth layer giữa "Rust catches most bugs" và "hardware fence catches the rest".
...
Insight sâu sắc nhất từ research: hầu hết OS **design for isolation first, sharing second** — kết quả là sharing selalu expensive (copy, syscall, shared memory boilerplate). ViCell đang đi đúng hướng với Grant API nhưng chưa đến cùng.
...
```
[ViCell Cell Loader — Security Audit]
Loading: camera-cell v1.3 (signed by ViCell Lab)
...
**Giá trị**: Đây là thứ không tồn tại ở bất kỳ OS hay package manager nào. npm/cargo audit check CVE, không check unsafe surface. ViCell có đủ thông tin (Cell manifest + linker) để làm điều này tại load time.
...
µFork dùng CHERI hardware để relocate pointers sau khi copy. ViCell không có CHERI, nhưng có thứ tương đương: **StateTransfer trait** đã serialize/deserialize Cell state.
...
**Caveat quan trọng**: µFork cần CHERI để handle raw pointers trong state. ViCell's approach chỉ work nếu `serialize_state()` không leak raw pointers — tức là Cell state phải serialize cleanly qua StateTransfer trait. Cells với raw pointer in state sẽ không support `spawn_fork`.
...
Claims từ Theseus OS documentation rằng Rust alone đủ cho LBI đều bị bác bỏ 0-3 hoặc 1-2. Tức là **ViCell không được claim Rust là đủ** — cần Memory Ownership Registry + unsafe budget enforcement như bổ sung.

### ❌ "ARM MPU thực tế không ai dùng vì overhead" — BÁC BỎ 3-0
Claim này từ một paper 2019 nhưng bị bác — thực tế Zephyr và FreeRTOS có MPU support và một số system dùng. ViCell không nên dùng argument "MPU too slow" — thay vào đó dùng argument đúng hơn: "MPU không cover DMA, còn LBI + AsyncLocked thì cover."

### ❌ "POSIX fork không thể làm trong SAS" — BÁC BỎ 3-0
µFork (SOSP 2025) chứng minh ngược lại. Điều này có nghĩa ViCell **không cần từ chối POSIX compatibility** vĩnh viễn — có path đến G2 POSIX fork support.
...
|---|---|---|---|---|
| **DMA Safety Lock API** | Zephyr RFC #60289 | Low | **G1 ngay** | Gap đã được xác nhận trong tất cả RTOS — ViCell có thể fix |
...
**DMA Safety Lock API** — kernel AsyncLocked đã có, chỉ cần wrap thành explicit `sys_dma_lock` syscall với `device_id` tracking. Khoảng 200-300 LOC kernel change. Ngay lập tức ViCell có thể claim: *"ViCell là RTOS đầu tiên có explicit DMA safety primitive không phụ thuộc IOMMU/SMMU"* — điều Zephyr đang cố làm từ 2023 và chưa xong.
