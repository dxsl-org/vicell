# Cellos vs. Embedded Systems — Competitive Analysis

**Version**: 1.0  
**Last Updated**: 2026-06-08  
**Scope**: Cellos G1 (Robot & Embedded) so sánh với các hệ thống nhúng phổ biến nhất

---

## 1. Tổng quan — Ma trận so sánh nhanh

| Tiêu chí | FreeRTOS | Zephyr | RTEMS | QNX | Eclipse ThreadX | Embassy (Rust) | **Cellos** |
|---|---|---|---|---|---|---|---|
...
### Khi nào FreeRTOS thắng Cellos
- MCU Cortex-M0/M3 dưới 64KB RAM (Cellos không support)
...
### Khi nào Cellos thắng RTEMS
- Robot thương mại cần fast iteration (Cellos không cần cert process)
...
### Khi nào QNX thắng Cellos
- Automotive IEC 26262 ASIL-D bắt buộc (QNX đã certified, Cellos chưa)
- Medical FDA cleared device
- Team đã có QNX expertise + budget license
- Cần Linux app compatibility layer (QNX có)

### Khi nào Cellos thắng QNX
- **IPC performance**: vtable call ~2-3 cycles vs QNX message queue ~microseconds — Cellos thắng tuyệt đối trên AI/sensor pipeline
- **Cost**: Cellos MIT, QNX tốn license
- **Rust ecosystem**: Cellos native Rust, QNX C/C++
...
- **Direct module call**: giống Cellos về IPC performance trong module-to-module call
...
- **Scale**: cùng Cell model từ MCU (Cellos-Nano) → SBC → server
- **OTA không downtime** sau khi deploy

---

## 8. Cellos — Điểm mạnh và giới hạn thực tế
...
Cellos-Nano (RV32 MCU) → Cellos G1 (ARM64/RV64 SBC) → Cellos G2 (x86/server). Cùng Cell API, cùng IPC, cùng deployment model. Robot với MCU satellite node + SBC brain + cloud backend có thể share code và abstraction xuyên suốt. Không hệ thống nào khác trong bảng có điều này.

**⑥ Tier 1b: Vendor SDK (C/C++) trong SAS**  
RKNN, Hailo, K230 KPU, legacy robot firmware C/C++ link statically vào Cell. POSIX shim (Cellos-libc) resolve `malloc/open/read` về ViSyscall. Zero overhead, zero rewrite. Embassy và FreeRTOS không có story sạch cho điều này.
...
| **MCU ecosystem nhỏ** | Cao | Cellos-Nano đang sub-track G1 |
...
Cellos không phải là "RTOS tốt hơn FreeRTOS". Đó là một paradigm khác:

> **FreeRTOS/Zephyr/Embassy** → "firmware đơn giản, cố định, ship nhanh"  
> **Cellos** → "hệ thống phức tạp, sống lâu, cần fault recovery, cần AI integration, cần uniform abstraction xuyên hardware tiers"

Điểm không thể thiếu của Cellos nằm ở giao điểm của ba yếu tố mà **không hệ thống nào khác đồng thời đáp ứng**:
1. Memory safety + LBI không cần MMU
2. Live fault recovery không reboot
3. Scalable từ MCU đến server với cùng Cell model

Khi sản phẩm robot đủ phức tạp để ba yếu tố trên trở thành yêu cầu thực sự — đó là lúc Cellos trở thành lựa chọn tự nhiên.
