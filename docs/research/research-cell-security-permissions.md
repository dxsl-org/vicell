# Cellos Cell Security: Permission Model + Hardware Attestation — Nghiên cứu
...
- **Entitlements** — quyền *tĩnh*, ký vào binary, là **trần** (ceiling) không vượt được. → **= ELF manifest hiện tại của Cellos.**
- **TCC** — đồng ý *runtime* do user kiểm soát, thu hồi được. → **= thứ Cellos đang thiếu.**
...
Cơ chế nền (manifest + kernel enforcement + scoping + revocation) tách rời UI consent. Cellos cần phần nền; dialog chỉ tùy chọn cho G2 HMI.

---

## 1. Trạng thái hiện tại của Cellos (thành thật)
...
Đã có (roadmap §G): Cell signing (planned), KMS Cell (planned), Silo (done, ARM64/x86 G2), audit ring ([audit.rs](../../kernel/src/audit.rs) — có event spawn, không có content hash), syscall allowlist `__Cellos_syscalls`.
...
| OS | Cơ chế cốt lõi | Bài học cho Cellos |
|----|----------------|---------------------|
...
**4 bất biến ở MỌI hệ** (Cellos hiện vi phạm cả 4):
...
- **TCC bypass qua injection** — CVE-2020-9771, CVE-2021-30713 (XCSSET lợi dụng grant của Zoom), CVE-2023-26818 (dylib injection vào process entitled). Gốc rễ: **TCC check binary, không verify code đang chạy** → inject sau khi đã cấp. → **Cellos mạnh hơn ở đúng điểm này**: LBI + Rust type system ngăn code injection ⇒ lỗ "permission laundering" của TCC **không tồn tại** trong Cellos.
- **Permission ≠ enforcement** — SE-Android (NDSS 2013): permission userspace vô nghĩa nếu thiếu MAC ở kernel. → Cellos enforce ở **syscall boundary (Law 1)** là đúng; consent phải **feed vào** điểm enforce đó.
...
### 2.4 Lộ trình tiến hóa cho Cellos (u8 bitmask = tầng 1/3)

**Tầng 1 — Parameterized capabilities** (ngay; additive qua ELF section mới `__Cellos_cap_args` → **KHÔNG cần Law 1**, backward-compat: vắng = full-scope):
```
...
**Tầng 3 — Runtime revocation** (sau, khi có use case): `CapHandle` kernel object; grantor gọi `sys_cap_revoke(handle)` → kernel set `task.cap = None`; syscall kế tiếp trả `ViError::CapRevoked`; Cell nhận `AppEvent::CapRevoked`. Đơn giản hơn seL4 CDT (Cellos chưa có cap-to-cap derivation).
...
| **Apple SEP** | (tham chiếu thiết kế) | ❌ | ❌ | **Silo Cellos = tương đương SEP** (coproc riêng, mailbox, UID không rời) |
...
**Kết luận:** Silo Cellos (Stage-2 fence, P-256, key an toàn dù kernel bị chiếm) **tương đương Apple SEP về kiến trúc**. Tiến hóa: giữ API `ostd::silo::SiloHandle`, đổi backend Stage-2 mailbox → **OpenTitan** (Earl Grey rời qua SPI, hoặc Darjeeling IP nhúng SoC G3). Silo hiện là xấp xỉ phần mềm; OpenTitan là hiện thực phần cứng.
...
Cellos có hook đúng chỗ ([`loader.rs spawn_from_path()`](../../kernel/src/loader.rs) đọc ELF trước spawn). Thiếu (~10 dòng tại hook đã có):
```
...
L2 Kernel     : Cellos verify bởi bootloader; CDI_kernel             ← THIẾU
...
|---|------|-------|---------|
| **P1** | Parameterized caps (`__Cellos_cap_args` section) | G1 | Additive, **no Law 1**; mở khóa GPIO pin / LBA range scoping ngay |
...
**Law 1:** nâng manifest (`u8`→`u16` / thêm field) đụng `libs/api/` ⇒ 2x confirmation. **Né:** dùng ELF section *mới* `__Cellos_cap_args` cho tham số (additive, backward-compat, không đụng struct ABI cũ).
