# Hardware-Assisted Isolation cho ViCell SAS — Nghiên cứu
...
**Hệ quả thiết kế (đồng thuận toàn ngành):** Pure LBI là đủ *khi và chỉ khi* mọi code do compiler tin cậy biên dịch từ ngôn ngữ verified-safe. Ngay khi nhận unsafe code (C driver/app, Lua FFI, native binary) → bổ sung phần cứng trở thành **bắt buộc**. ViCell ở đúng ranh giới này: Tier 1 Rust thuần dùng LBI; Tier 1b/3b cần supplement.
...
Kết luận corpus: **MPK là cơ chế hiệu năng, không phải security boundary — trừ khi đi kèm CFI (BTI/IBT) + binary scan.** Với ViCell, Rust thuần trong Cell trung hòa phần lớn; rủi ro quay lại ở C FFI (mlibc, DOOM), Lua dispatch, unsafe kernel — đúng chỗ gadget space = toàn bộ image.
...
> Trong ViCell SAS, **Cell CHÍNH LÀ driver** — nó own MMIO, tự lập trình DMA descriptor, không có kernel trung gian. Blast radius của một Cell-có-DMA bị compromise = **một bug driver kernel**, KHÔNG phải exploit user-space. Nó ghi/đọc *toàn bộ* physical memory (page table kernel, metadata scheduler, stack Cell khác) — không cần một dòng `unsafe`, chỉ cần ghi MMIO mà nó được phép ghi.

So với Linux: process user không đọc được physical memory của process khác kể cả khi không có IOMMU, vì nó không có MMIO tới DMA controller. ViCell Tier 1 Cell *là* driver. ⟹ DMA threat của ViCell ngang một **kernel driver bug của Linux**, không phải user exploit.
...
### 3.2 ⚠️ Trạng thái ViCell hiện tại: IOMMU passthrough = ZERO isolation
...
**Giới hạn 16 key** → nhóm theo **tier** (Key1=kernel-trusted, Key2=service, Key3=app/untrusted) khớp mô hình tier ViCell; hoặc multiplex kiểu **libmpk** (có chi phí eviction). **Đừng** per-Cell key.
...
- **Tock OS** (Rust nhúng): **capsule** (trusted, in-kernel) = Rust type system, **zero** MPU; **process** (untrusted, C app) = ARM MPU / RISC-V PMP reconfigure mỗi context switch. ⭐ Đây là mô hình map thẳng cho ViCell: Tier 1 Rust thuần = capsule (đừng trả giá MPU vô ích); Tier 1b (C/Lua/DOOM) = process (gate sau MPU/PMP). "Grant" của Tock: kernel cấp heap *bên trong* memory của process nhưng MPU chặn process đọc — đúng pattern owned-buffer.
...
- **Hubris** (Oxide): task tĩnh compile-time, MPU region baked vào image, reconfigure khi switch. Oxide nói thẳng: *"kể cả Rust, memory protection vẫn thiết yếu."* IPC "lease" = đúng Law 2 owned-buffer; supervisor restart = đúng mô hình never-die ViCell.
  - Nguồn: <https://hubris.oxide.computer/reference/> · <https://oxide.computer/blog/hubris-and-humility>

### 5.2 PMP dưới Bare mode (tin tốt cho ViCell)

Theo RISC-V priv spec: `sfence.vma` chỉ cần sau khi ghi PMP CSR **nếu page-based VM được implement**. Dưới `satp=Bare` (ViCell SAS) → ghi PMP **không cần** sfence.vma → SAS-safe. Nhược: O(N) ghi CSR cho N entry mỗi switch.
...
- **Liên hệ ViCell:** VMM custom ~9K-LOC hiện tại phù hợp G1/G2 bare-metal. Nên thiết kế **`VmHandle` ABI trung lập** ngay để sau backing bằng TDX/SEV-SNP/CCA mà không redesign protocol Cell↔VM — mở khóa **remote attestation** (mở rộng nguyên tắc Silo cho multi-tenant).

---

## 8. CHERI — endgame phần cứng cho SAS

CHERI capability thay **cả** MMU-isolation **lẫn** language-isolation: mọi pointer là (address, bounds, permissions, tag); hardware check mọi load/store; tag chỉ set bởi hardware (không giả được). **CheriOS** (Cambridge 2021) đã chứng minh đúng mô hình ViCell: MMU dùng cho hiệu năng (TLB cache), capability dùng cho isolation, **không TLB flush khi switch domain**, chạy được cả C.
...
| OS | Cô lập bằng | Phần cứng | Bài học cho ViCell |
|----|-------------|-----------|--------------------|
...
| **RedLeaf** (OSDI'20) | Rust domain, no page-table | Không | **RRef\<T\>** ownership enforce bởi IDL → cân nhắc ViCell IDL |
| **Theseus** (OSDI'20) | LBI thuần, Ring 0 | Không (cố ý) | Cảnh báo: không chạy được C bên thứ ba — Tier 1b/3b ViCell trung thực hơn |
