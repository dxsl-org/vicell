...
Những gì ViCell cần học từ các dự án OS khác, kèm nguồn và repo để triển khai lần lượt.

---

## 1. Tock OS — Language-Based Isolation cho MCU

**Tại sao quan trọng**: Closest analogue to ViCell. Tock's capsule ≈ ViCell Cell. Production-deployed (OpenTitan, Chromebook EC).

**Repo**: https://github.com/tock/tock
**Docs**: https://book.tock.os/

### 1.1 Grant Mechanism → ViCell Phase 26
...
```rust
// Tock pattern (học để làm ViCell per-cell memory quota):
...
**ViCell action (Phase 26)**: Implement `MemoryQuota` từ `GlobalAlloc` caller-PC tracking (spec 02-memory.md §2). Tock source để đọc: `kernel/src/grant.rs`.

### 1.2 Syscall Filter Per Cell → ViCell Phase 27
...
**ViCell action (Phase 27)**: Thêm `allowed_syscalls: &[u32]` vào Cell ELF metadata, kernel enforce tại `sys_call_dispatch()`.
**Tock source**: `kernel/src/process.rs`, `capsules/core/`.

### 1.3 Capsule Panic Isolation → ViCell Phase 26

Capsule panic → chỉ kill capsule, kernel + other capsules tiếp tục. Tock dùng `catch_unwind`-equivalent per capsule.

**ViCell action**: ViCell spec (01-core.md) đã có `catch_unwind`. Cần wire vào mọi Cell dispatch path, không chỉ kernel-level panic.
...
**Tại sao quan trọng**: Production Rust RTOS cho RoT (Root of Trust) trong Oxide Computer / Azure Sphere. IPC model gần giống ViCell spec.

**Repo**: https://github.com/oxidecomputer/hubris
**Book**: https://hubris.oxide.computer/

### 2.1 Synchronous Call-Return IPC ("Humility") → ViCell Phase 27
...
**ViCell relevance**: ViCell hiện dùng `sys_call` (synchronous) và `sys_send`/`sys_recv` (async). Hubris chứng minh synchronous-only IPC đủ cho production và đơn giản hơn nhiều.
**Hubris source**: `sys/kern/src/task.rs` → `send_receive()`.

### 2.2 Lease System (auto-expiring memory loans) → ViCell Phase 27
...
```rust
// Hubris lease pattern (ViCell cần implement tương tự):
...
**ViCell action**: Thay thế current `CapId` opaque token bằng `Lease` type với auto-revoke. Hubris source: `sys/kern/src/lease.rs`.

---

## 3. RTIC v2 — Priority Scheduler Design

**Tại sao quan trọng**: Static priority + hardware preemption, zero runtime overhead. Pattern cho ViCell Phase 25.

**Repo**: https://github.com/rtic-rs/rtic
**Book**: https://rtic.rs/2/book/en/

### 3.1 Static Priority Assignment → ViCell Phase 25

```rust
// RTIC pattern — ViCell Phase 25 TaskPriority enum nên học theo:
...
**ViCell action**: `TaskPriority { RealTime=0, Normal=1, Background=2 }`. Preempt lower-priority khi higher-priority becomes runnable. RTIC source: `rtic-macros/src/codegen/dispatchers.rs`.

### 3.2 Software Task Dispatching → ViCell Phase 25

RTIC không dùng timer tick cho software tasks — dùng **pending interrupt** để trigger dispatch. Zero overhead khi không có work.

**ViCell action**: Thay vì round-robin timer tick 10ms, dùng RISC-V software interrupt để wake specific priority level khi có task ready. RTIC source: `rtic-sw-pass/src/`.
...
### 4.2 SMP Per-CPU Scheduler → ViCell Phase 32

Hermit có per-CPU run queue với work stealing. Relevant khi ViCell cần multi-core support.

**Hermit source**: `src/scheduler/mod.rs`, `src/scheduler/task.rs`.

---

## 5. RedLeaf — Academic Validation của ViCell Thesis
...
**ViCell relevance**: RedLeaf = peer-reviewed proof of concept cho ViCell thesis. Khi cần cite academic validation, dùng bài này.
...
**ViCell action**: Đọc bài này trước khi thiết kế Phase 26 (memory quota) và Phase 28 (WASM isolation). Source: link ở trên.
...
### 6.1 RISC-V ePMP Thay Vì Intel MPK → ViCell Phase 28
...
**ViCell action (Phase 28)**: Thay vì chờ CHERI, dùng **RISC-V ePMP** để enforce Cell boundaries. Mỗi Cell = 1 PMP entry. Violation → trap → kernel isolate cell.
...
### 7.1 Typed IPC Channels → ViCell Phase 27
...
**ViCell action**: Định nghĩa IPC channel types trong `libs/api/` dùng Rust enums. Compile-time check thứ tự protocol.

```rust
// ViCell typed channel concept (Phase 27):
pub enum VfsRequest {
...
### 7.2 Capability Manifests trong ELF → ViCell Phase 29

Mỗi SIP khai báo capabilities trong manifest trước khi load. Kernel verify tại load time.

**ViCell action**: Thêm `.ViCell_manifest` ELF section với capability declaration:
```toml
...
### 8.1 Escape Hatch — Lesson From Failure → ViCell Phase 28 (Critical)

Midori/Singularity không thể chạy legacy software → zero adoption path → dự án chết.

**ViCell action**: Tier 3 VM (Phase 28) phải implement **trước** khi release ViCell cho external users. Không có VM = không có escape hatch = same fate as Midori.

### 8.2 ZST Capabilities-as-Types → ViCell Phase 26
...
ViCell spec (01-core.md) đã mô tả ZST capability tokens — **chưa implement**.

```rust
// ViCell ZST capability pattern (Phase 26) — học từ Midori:
...
**Relevance**: Embassy's async executor là state-of-the-art cho bare-metal Rust. ViCell's ostd executor nên học từ đây.

### 9.1 Interrupt-Driven Wakers → ViCell smoltcp improvement

Embassy integrate async executor với hardware interrupts — khi IRQ fires, executor wake đúng task đang chờ event đó.

```rust
// Embassy pattern — ViCell network polling nên làm tương tự:
...
**ViCell action**: Thay smoltcp busy-poll loop bằng IRQ-driven waker. Embassy source: `embassy-net/src/lib.rs` → `poll()`.
...
| **29** | Singularity | Cell capability manifests trong ELF `.ViCell_manifest` | [SOSP 2007 paper](https://www.microsoft.com/en-us/research/publication/singularity-rethinking-the-software-stack/) |
